...
Param(
    ...
    [Parameter(Mandatory = $True, ParameterSetName='PSModuleBuild')]
    [string]$ManifestPath,
    [Parameter(Mandatory = $True, ParameterSetName='PSModuleBuild')]
    [string]$PSModuleBuildJSONPath,
...
if (($ManifestPath.EndsWith('psd1')) -and (Test-Path $ManifestPath)) {
    ...
    switch ($PSCmdlet.ParameterSetName) {
        ...
        'PSModuleBuild' {
            # Passed a manifest name and a psmodulebuild json configuration file to validate
            if (($PSModuleBuildJSONPath.EndsWith('psd1')) -and (Test-Path $PSModuleBuildJSONPath)) {
                try {
                    $ModuleInfo = get-content $PSModuleBuildJSONPath | ConvertFrom-Json
                }
                catch {
                    throw "$PSModuleBuildJSONPath either does not exist or is not a json file!"
                }
            }
            else {
                throw "$PSModuleBuildJSONPath either does not exist or is not a json file!"
            ...
            # Passed a manifest name and several manual entries to validate
            Describe 'Module Manifest - PsModuleBuild Tests' {
                Context "Testing $ManifestPath" {
                    if (-not [string]::IsNullOrEmpty($Version)) {
                        It "should be module version '$Version'" {
